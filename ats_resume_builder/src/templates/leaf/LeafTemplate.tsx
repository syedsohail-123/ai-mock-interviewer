...
export const LeafTemplate = ({ data, metadata }: TemplateProps) => {
  const { basics } = data;
  ...
  return (
    <div
      ...
      style={{
        ...
        color: metadata.colors.text || '#1e293b',
    ...
    >
      {/* Refined Leaf Header with Accent Top Border */}
      <header className="p-8 pb-5 flex flex-col items-center text-center gap-3.5 border-b-2 border-slate-800">
          ...
          <div className="flex flex-col items-center">
            ...
            <p
              style={{
                ...
                fontSize: `${metadata.typography.bodySize * 1.25}pt`,
              }}
        ...
        {/* Clean Editorial Horizontal Centered Contact Bar */}
        <div className="flex flex-wrap justify-center items-center gap-x-5 gap-y-2 text-xs border-t border-slate-200/80 pt-3 text-slate-700 w-full">
          {basics.email && (
            <ContactItem
              icon={<Mail size={13} />}
              value={basics.email}
      ...
      {/* Main Body - Fluid Two Columns with Sidebar on Left */}
